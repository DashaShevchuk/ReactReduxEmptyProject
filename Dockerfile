...
WORKDIR /src
COPY ["LaptopShop/LaptopShop.csproj", "LaptopShop/"]
RUN dotnet restore "LaptopShop/LaptopShop.csproj"
COPY . .
WORKDIR "/src/LaptopShop"
RUN dotnet build "LaptopShop.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "LaptopShop.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "LaptopShop.dll"]